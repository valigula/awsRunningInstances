...
def get_aws_prices(instance_type):
    ...
    try:
        ret=""
        cur = con.cursor()
        cur.execute("select price from awsprices where type='"+instance_type+"'and region = 'eu-west-1' and reserved_od='ondemand' and os  = 'linux' limit 1")
        result = cur.fetchall()
        for row in result:
            ret = row[0]
            continue
...
def set_curl(stats):
    print 'curl -X POST -d \'[{"name":"ec2_desc","columns":["InstanceId", "InstanceType","PrivateIpAddress", "Time" , "Price","SecurityGroup", "Status"], "points":[["'+stats["InstanceId"]+'","'+stats["InstanceType"]+'","'+ stats["PrivateIpAddress"]+'","'+stats["Time"]+'", '+stats["Price"]+', "'+stats["SecurityGroup"]+'" , "'+stats["Status"]+'"  ]]}]'+"\', "+ '\'http://path_to_influx:8086/db/mydb/series?u=root&p=root\';'
